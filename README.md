...
- **Sequence models** window inside each fold. The first `seq_len - 1` bars of every test fold get no prediction and sit flat. That costs you a little OOS coverage and buys you a guarantee that no window straddles the split.
...
- **Re-tuning per fold** is a random search on the chronological tail of each training fold. It is the honest option and it multiplies training time by the number of candidates.
